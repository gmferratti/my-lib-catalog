...
| `etiquetas` | str | `"lazer, doutorado"` | Tags livres separadas por vírgula; vazio se nenhuma |
...
### Normalização de títulos de série

`catalog/series.py` expõe duas funções públicas:

- `detectar_serie(titulo: str) -> dict | None` — detecta padrões de volume (`Vol. N`, `V. N`, `Tomo N`, numerais romanos) e retorna `{"serie", "volume", "subtitulo"}` ou `None`.
- `compor_titulo(serie, volume, subtitulo) -> str` — produz o formato canônico `Série — Vol. N: Subtítulo`.

`salvar()` em `catalog/storage/persistence.py` chama `detectar_serie` automaticamente antes de gravar; qualquer título com padrão de volume é normalizado na entrada.

O dialog de edição em `ui/utils.py` tem um toggle "É parte de uma série" que, quando ativado, exibe campos de Série / Vol. nº / Subtítulo e recompõe o `titulo` ao salvar. O toggle é ativado automaticamente se `detectar_serie` detectar padrão no título atual.
...
Campo `etiquetas` no schema: string com tags separadas por vírgula (mesmo padrão de `assuntos`). Editável via multiselect com `accept_new_options=True` no dialog de edição. Exibido como badges roxos nos cards do acervo (máx. 3 visíveis + "+N") e na ficha do livro. Filtrável na sidebar com lógica AND. Helper `_badge_etiqueta()` em `ui/utils.py` gera o HTML do badge.
